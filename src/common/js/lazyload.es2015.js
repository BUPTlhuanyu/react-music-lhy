...
const setWasProcessedData = element => setData(element, processedDataName, trueString);

const getWasProcessedData = element => getData(element, processedDataName) === trueString;
...
const replaceExtToWebp = (value, condition) => (condition ? value.replace(/\.(jpe?g|png)/gi, '.webp') : value);
...
//判断是否是爬虫
const isBot =
    (runningOnBrowser && !('onscroll' in window)) || /(gle|ing|ro)bot|crawl|spider/i.test(navigator.userAgent);
...
const setSourcesInChildren = function(parentTag, attrName, dataAttrName, toWebpFlag) {
...
const setAttributeIfValue = function(element, attrName, value, toWebpFlag) {
    if (!value) {
        return;
    }
