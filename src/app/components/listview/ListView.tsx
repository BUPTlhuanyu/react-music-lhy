...
interface ListViewProps {
    ...
    listClassName?: string | Record<string, any>;
}
...
const ListView = function (props: ListViewProps) {
    ...
    const listviewClassName = classnames('listview', props.listClassName);
    ...
    return (
        <div className={listviewClassName} ref={scrollContanier}>
            <ul ref={listGroup}>
                {!!props.data.length
                    && props.data.map((group: {title: string, items: any[]}) => (
                                ...
                                {!!group.items.length
                                    && group.items.map((item: IItem) => (
                                        <li
                                            ...
                                            key={item.id}
                                            onClick={props.getItem.bind(null, item)}
                                        >
                                            <LazyImage
                ...
                <ul>
                    {!!shortcutList.length
                        && shortcutList.map((item, index) => {
                            ...
                            return (
                                <li className={className} key={index} data-index={index}>
                                    {item}
                                </li>
                            );
                        })}
