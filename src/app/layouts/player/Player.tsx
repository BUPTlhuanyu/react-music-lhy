...
import {setFullScreen, setPlaying, setCurrentIndex, setPlayMode, setPlaylist, setSequenceList} from 'actions/player';
...
interface PlayerPropType {
    ...
    setSequenceList: (songs: ISong[]) => void;
...
class Player extends Component<PlayerPropType, PlayerStateType> {
    ...
    toggleFavorite(item: any) {
        ...
        console.log(
            '可以判断当前歌曲是否存在于喜欢的歌曲列表中来决定这里favorite状态，建议直接在render中判断，尽量避免使用getDerivedStateFromProps'
        );
    ...
    handleLyric = ({lineNum, txt}: {lineNum: number, txt: string}) => {
        ...
        // console.log("txt",txt)
        if (!this.lyricList.current || !this.lyricLine.current) {
            return;
        }
        this.setState({
    ...
    iconMode = () => {
        return this.props.mode === playMode.sequence
            ? 'icon-sequence'
            : this.props.mode === playMode.loop
                ? 'icon-loop'
                : 'icon-random';
    };
    ...
    middleTouchMove: React.TouchEventHandler<HTMLDivElement> = e => {
        if (
            ...
            || !this.lyricList.current
            || !this.middleL.current
            || !this.lyricList.current.wrapper.current
        ) {
            return;
        }
        const touch = e.touches[0];
    ...
    modeText = () => {
        return this.props.mode === playMode.sequence
            ? '顺序播放'
            : this.props.mode === playMode.random
                ? '随机播放'
                : '单曲循环';
    };
        ...
        return (
            <div className="player">
                    ...
                    <div className="normal-player">
                        ...
                        <div className="bottom">
                            ...
                            <div className="operators">
                                ...
                                <div className={this.disableCls() + ' icon i-right'}>
                                    <i
                                        className={favorite ? 'icon-favorite' : 'icon-not-favorite'}
                                        onClick={() => {
                ...
                <PlayList
                    ref={this.playlist}
                    toggleFavorite={() => {
                        console.log('todo，歌曲的favorite信息应该存在store中');
                    }}
